const getVariant = (variant?: VariantType) => {
  switch (variant) {
    case "primary":
      return "bg-black text-white"; // Changed to black background and white text
    case "danger":
    ...
    default:
      return "bg-black text-white"; // Default to black background and white text
  }
};
  ...
  return (
    <button
      ...
      className={`
        ...
        ${!paddingLess && "py-2 px-4"} ${!square && "rounded-md"} active:scale-95 ${className}
      `}
    >
      {children}
    </button>
